...
// token verification / perm secret
app.post('/api/verify'),
	(req, res) => {
		...
		try {
			...
			console.log(error);
			res.status(500).json({ message: `error finding user` }); // If any errors show this
		}
	};

app.listen(port, () =>
	console.log(`Server is currently running and listening on port ${port}`)
); //: Sets a message in the console if server is up and running and port is open
